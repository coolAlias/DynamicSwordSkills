...
public class ButtonTextElement extends GuiTextElement
{
	...
	public ButtonTextElement(GuiContainedButton button, ITextComponent text) {
		super(button.x, button.y, button.width, button.height, text, button.getDefaultLabelColor(), button.isUnicode);
		this.parent = button;
	...
	protected void drawLine(Minecraft mc, int mouseX, int mouseY, final String line, final int x, final int y, boolean hovered) {
		...
		if (this.parent.drawLabelShadow) {
			mc.fontRenderer.drawStringWithShadow(line, (float)x, (float)y, this.parent.getLabelColor());
		} else {
