...
public class GuiTextElement extends GuiContainedElement
{
	...
	public void setScrollableArea(int width, int height) {
		...
		if (this.lines == null) {
			...
			mc.fontRendererObj.setUnicodeFlag(this.isUnicode);
			...
			Stream.of(this.text.getUnformattedText().split("\\\\n")).forEach(line -> {
				mc.fontRendererObj.listFormattedStringToWidth(line, this.remainingWidth).stream().forEach(s -> {
					this.lines.add(formatting + s + TextFormatting.RESET);
				});
			});
			this.contentHeight = (this.lines.size() * mc.fontRendererObj.FONT_HEIGHT);
	...
	public void drawElement(Minecraft mc, int mouseX, int mouseY) {
		...
		if (this.scrollY > 0) {
			...
			while (this.scrollY >= mc.fontRendererObj.FONT_HEIGHT && start < this.lines.size()) {
				start++;
				this.scrollY -= mc.fontRendererObj.FONT_HEIGHT;
			}
			// Account for partially scrolled line
			if (start < this.lines.size() && this.scrollY > 0 && this.scrollY < mc.fontRendererObj.FONT_HEIGHT) {
				start++;
				pY += mc.fontRendererObj.FONT_HEIGHT - this.scrollY;
				this.scrollY -= mc.fontRendererObj.FONT_HEIGHT;
			}
		...
		int hoverHeight = Math.min(this.remainingHeight, this.padding.bottom + mc.fontRendererObj.FONT_HEIGHT * (this.lines.size() - start));
		...
		for (int i = start; i < this.lines.size() && (this.displayedHeight + mc.fontRendererObj.FONT_HEIGHT) <= this.remainingHeight; ++i) {
			String s = this.lines.get(i);
			int x = (this.isTextCentered ? innerX + ((this.remainingWidth - this.padding.width()) / 2) - (mc.fontRendererObj.getStringWidth(s) / 2) : innerX);
			int y = this.yPos + this.displayedHeight;
			this.drawLine(mc, mouseX, mouseY, s, x, y, hovered);
			this.displayedHeight += mc.fontRendererObj.FONT_HEIGHT;
		}
	...
	protected void drawLine(Minecraft mc, int mouseX, int mouseY, final String line, final int x, final int y, boolean hovered) {
		if (this.forceHover) {
			if (this.drawTextShadow) {
				mc.fontRendererObj.drawStringWithShadow(line, (float)x, (float)y, this.hoverColor);
			} else {
				mc.fontRendererObj.drawString(line, x, y, this.hoverColor);
			}
		} else if (!hovered || !this.isHoverable) {
			mc.fontRendererObj.drawString(line, x, y, this.color);
