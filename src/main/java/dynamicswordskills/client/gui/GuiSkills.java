...
public class GuiSkills extends GuiScreen
{
	...
	public void initGui() {
		...
		this.skills = DSSPlayerInfo.get(this.mc.player);
		...
		this.buttonList.add((new PageButtonImage(btns++, this.guiLeft + 12, this.guiTop + 158, 16, paginationHeight, new TextComponentTranslation("skill.dss.gui.button.label.prev").getUnformattedText(), GUI_TEXTURE, 300, 180, 284, 18, 16, 13) {
			@Override
		...
		this.buttonList.add((new PageButtonImage(btns++, this.guiLeft + 253, this.guiTop + 158, 16, paginationHeight, new TextComponentTranslation("skill.dss.gui.button.label.next").getUnformattedText(), GUI_TEXTURE, 300, 180, 284, 31, 16, 13) {
			@Override
			public int getPageIndex(int current, int numPages) {
		...
		int i = 1;
		this.buttonList.add(new IndexButton(btns++, this.guiLeft + 154, this.guiTop + 55, w, h, new TextComponentTranslation("skill.dss.gui.button.label.main").getUnformattedText(), 0).pad(ip, 0).setDrawButtonBox(!plainText));
	...
	protected void initPages() {
		...
		for (SkillGroup group : SkillGroup.getAll()) {
			List<SkillBase> skills = group.getSkills((t) -> {
	...
	protected Page<?> getTitlePage() {
		...
		String headerText = new TextComponentTranslation("skill.dss.gui.main.title").getUnformattedText().toUpperCase();
		GuiTextElement header = new GuiTextElement(42, 26, 80, 9, new TextComponentTranslation(headerText), TEXT_COLOR, false);
		GuiElementContainer<GuiTextElement> body = new GuiTextElementContainer(18, 32, 110, 127).setElementPadding(0, mc.fontRendererObj.FONT_HEIGHT);
		body.pad(7, 0, 7, 5);
		body.add(((GuiTextElement) new GuiTextElement(body, new TextComponentTranslation("skill.dss.gui.main.body"), TEXT_COLOR, true).pad(0)));
	...
	protected Page<?> getTableOfContents() {
		...
		String headerText = new TextComponentTranslation("skill.dss.gui.heading.index").getUnformattedText().toUpperCase();
		GuiTextElement header = new GuiTextElement(153, 24, 104, 13, GuiTextElement.getBoldComponent(new TextComponentString(headerText), null), TEXT_COLOR, true);
	...
	protected Page<SkillSlot> getSkillContainerPage(SkillSlotContainer body) {
		...
		GuiTextElement header = new GuiTextElement(42, 24, 79, 9, GuiTextElement.getBoldComponent(new TextComponentString(headerText), null), TEXT_COLOR, true);
	...
	protected Page<?> getSkillDescriptionPage(SkillBase skill) {
		...
		GuiElement header = new GuiCompositeElement(153, 24, 106, 9, 
				new GuiTextElement(153, 24, 96, 9, GuiTextElement.getBoldComponent(new TextComponentTranslation(skill.getDisplayName().toUpperCase()), TextFormatting.DARK_GRAY), TEXT_COLOR, true),
				new GuiTextElement(250, 24, 11, 9, GuiTextElement.getBoldComponent(new TextComponentString(lvl), TextFormatting.DARK_GRAY), TEXT_COLOR, true).setCentered(true)
				);
		GuiElementContainer<GuiTextElement> body = new GuiTextElementContainer(153, 35, 110, 121);
		body.pad(2, 5, 2, 3);
		if (Config.isSkillDisabled(this.mc.player, currentSkill)) {
		...
		body.add(new GuiTextElement(body, GuiTextElement.getBoldComponent(new TextComponentTranslation("skill.dss.gui.heading.summary"), null), TEXT_COLOR, true));
		List<String> details = Lists.<String>newArrayList();
		details.add(this.currentSkill.getLevelDisplay(false));
		this.currentSkill.addInformation(details, this.mc.player);
		details.stream().forEach(s -> body.add(new GuiTextElement(body, new TextComponentString(s), TEXT_COLOR, true)));
		// Activation requirements, if any
		String activation = this.currentSkill.getActivationDisplay();
		if (activation != null) {
			body.add((GuiTextElement)(new GuiTextElement(body, GuiTextElement.getBoldComponent(new TextComponentTranslation("skill.dss.gui.heading.activation"), null), TEXT_COLOR, true).pad(pad, 0, 0, 0)));
			body.add(new GuiTextElement(body, activation, TEXT_COLOR, true));
		}
		// Full description
		body.add((GuiTextElement)(new GuiTextElement(body, GuiTextElement.getBoldComponent(new TextComponentTranslation("skill.dss.gui.heading.description"), null), TEXT_COLOR, true).pad(pad, 0, 0, 0)));
		body.add(new GuiTextElement(body, new TextComponentTranslation(this.currentSkill.getTranslationKey() + ".description"), TEXT_COLOR, true));
	...
	protected void setPageLeft(int index) {
		index = MathHelper.clamp(index, 0, this.pages.size() - 1);
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		this.buttonList.get(0).visible = this.pageLeft.index > 0;
		this.buttonList.get(1).visible = this.pages.size() > 0 && this.pageLeft.index + 1 < this.pages.size();
		...
		for (GuiButton button : this.buttonList) {
			if (!button.visible || !button.isMouseOver()) {
				...
				if (tooltip != null && !tooltip.isEmpty()) {
					GuiUtils.drawHoveringText(tooltip, mouseX, mouseY, this.width, this.height, 152, this.mc.fontRendererObj);
				}
			}
			break;
		}
	...
	public static class PageNumberFooter extends GuiElement
	{
		...
		public void drawElement(Minecraft mc, int mouseX, int mouseY) {
			if (this.numPages > 1) {
				...
				mc.fontRendererObj.drawString(s, this.xPos - (mc.fontRendererObj.getStringWidth(s) / 2), this.yPos, 4210752);
			}
		}
	}
	...
	public static class SkillSlotContainer extends GuiElementContainer<SkillSlot>
	{
		...
		public void clearFilters() {
			this.filters.clear();
			this.elements.stream().forEach(s -> s.setDisabled(false));
			this.markDirty();
		}
		...
		protected void renderToolTip(SkillBase skill, Minecraft mc, int mouseX, int mouseY) {
			...
			if (skill.getLevel() < 1) {
				String displayName = (skill.showNameIfUnknown(mc.player) ? skill.getDisplayName() : new TextComponentTranslation("skill.dss.unknown.name").getUnformattedText());
				tooltip.add(displayName);
				if (Config.isSkillAllowed(skill)) {
					tooltip.add(TextFormatting.GRAY + (TextFormatting.ITALIC + new TextComponentTranslation("skill.dss.unknown.tooltip").getUnformattedText()));
				} else {
				...
				skill.getTooltip(this.skillScreen.mc.player, false).stream().forEach(s -> tooltip.add(TextFormatting.GRAY + s));
			}
			this.skillScreen.drawHoveringText(tooltip, mouseX, mouseY, mc.fontRendererObj);
