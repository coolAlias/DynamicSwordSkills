...
public class SkillSlot extends GuiContainedElement
{
	...
	public void drawElement(Minecraft mc, int mouseX, int mouseY) {
		boolean grayScale = Config.isSkillDisabled(mc.player, this.skill);
		int rgb = (grayScale ? 136 : 255);
		this.frame.setColor(rgb, rgb, rgb, 255);
		this.frame.drawElement(mc, mouseX, mouseY);
		if (this.isSkillKnown || this.skill.showIconIfUnknown(mc.player)) {
	...
	public static class TitledSkillSlot extends SkillSlot
	{
		...
		public TitledSkillSlot(SkillBase skill, int xPos, int yPos, int width, int height) {
			...
			if (this.skill == null) {
				...
				this.titleKnown = new GuiTextElement(xPos, yPos, this.width, this.height, new TextComponentTranslation(this.skill.getNameTranslationKey()), GuiSkills.TEXT_COLOR, false).setHoverable(true).setHoverColor(GuiSkills.HOVER_COLOR).setDrawTextShadown(true);
				this.titleUnknown = new GuiTextElement(xPos, yPos, this.width, this.height, new TextComponentTranslation("skill.dss.unknown.name"), GuiSkills.TEXT_COLOR, false);
			}
		}
		...
		public void drawElement(Minecraft mc, int mouseX, int mouseY) {
			if (this.isTitleVisible && this.skill != null) {
				GuiTextElement title = (this.isSkillKnown || this.skill.showNameIfUnknown(mc.player) ? this.titleKnown : this.titleUnknown);
				title.setHoverable(this.isSkillKnown);
				int ht = (title.getContentHeight() - title.getPadding().height()) / 2;
				int dy = (mc.fontRendererObj.FONT_HEIGHT - ht);
