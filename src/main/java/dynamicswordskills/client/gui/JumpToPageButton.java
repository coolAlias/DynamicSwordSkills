...
public class JumpToPageButton extends PageButtonText
{
	...
	public static class JumpToPageTextElement extends ButtonTextElement
	{
		...
		protected void drawLine(Minecraft mc, int mouseX, int mouseY, final String line, final int x, final int y, boolean hovered) {
			...
			int w = this.parent.getPadding().right + mc.fontRendererObj.getStringWidth(s);
			if (this.parent.drawLabelShadow) {
				mc.fontRendererObj.drawStringWithShadow(s, (float)(x + this.width - w), (float)y, this.parent.getLabelColor());
				mc.fontRendererObj.drawStringWithShadow(line, (float)(x + this.parent.getPadding().left), (float)y, this.parent.getLabelColor());
			} else {
				mc.fontRendererObj.drawString(s, x + this.width - w, y, this.parent.getLabelColor());
				mc.fontRendererObj.drawString(line, x + this.parent.getPadding().left, y, this.parent.getLabelColor());
			}
		}
	}
}
