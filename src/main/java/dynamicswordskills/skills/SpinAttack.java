...
public class SpinAttack extends SkillActive
{
	...
	public void keyPressed(KeyBinding key, EntityPlayer player) {
		if (key == DSSKeyHandler.keys[DSSKeyHandler.KEY_ATTACK] || key == Minecraft.getMinecraft().gameSettings.keyBindAttack) {
			...
			if (wasKeyPressed) {
				...
				clockwise = (key == DSSKeyHandler.keys[DSSKeyHandler.KEY_RIGHT] ||
