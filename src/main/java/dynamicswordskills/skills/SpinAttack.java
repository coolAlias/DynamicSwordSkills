...
public class SpinAttack extends SkillActive implements IModifiableSkill, IReachAttackSkill
{
	...
	private void startSpin(EntityPlayer player) {
		++refreshed;
		if (player.getEntityWorld().isRemote) {
			targets = player.getEntityWorld().getEntitiesWithinAABB(EntityLivingBase.class, player.getEntityBoundingBox().expand(getRange(), 0.0D, getRange()), EntitySelectors.IS_ALIVE);
