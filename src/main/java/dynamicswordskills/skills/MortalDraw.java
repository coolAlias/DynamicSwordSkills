	...
	public boolean activate(World world, EntityPlayer player) {
		if (attackTimer == 0) {
			...
			for (int i = 0; i < 9; ++i) {
				...
				if (stack != null) {
					if (stack.getItem() instanceof ISkillProvider && ((ISkillProvider) stack.getItem()).getSkillId(stack) == getId()) {
						swordSlot = i;
						break;
					} else if (PlayerUtils.isSwordItem(stack.getItem()) && swordSlot == -1) {
						swordSlot = i;
					}
				}
			}
