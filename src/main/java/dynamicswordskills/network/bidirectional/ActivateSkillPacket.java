...
public class ActivateSkillPacket implements IMessage
{
	...
	/*
	public static class ActivateSkillPacketHandler extends AbstractBiMessageHandler<ActivateSkillPacket> {
		// TODO verify whether client side handling is needed
		@Override
		public IMessage handleClientMessage(EntityPlayer player, ActivateSkillPacket message, MessageContext ctx) {
			if (DSSPlayerInfo.get(player) != null) {
				if (message.wasTriggered) {
					DSSPlayerInfo.get(player).triggerSkill(player.worldObj, message.skillId);
				} else {
		...
		@Override
		public IMessage handleServerMessage(EntityPlayer player, ActivateSkillPacket message, MessageContext ctx) {
			if (DSSPlayerInfo.get(player) != null) {
				if (message.wasTriggered) {
					DSSPlayerInfo.get(player).triggerSkill(player.worldObj, message.skillId);
				} else {
...
/*
public class ActivateSkillPacket extends AbstractPacket
{
	/** If true, calls triggerSkill(), otherwise uses activateSkill()
	private boolean wasTriggered = false;
