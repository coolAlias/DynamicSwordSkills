...
public class SkillGroup
{
	...
	public String getDisplayName() {
		return (this.displayName == null ? new TextComponentTranslation(this.getTranslationKey()).getUnformattedText() : this.displayName);
	}
	...
	public List<String> getTooltip() {
		...
		return Lists.<String>newArrayList(
				this.getDisplayName(),
				TextFormatting.GRAY + new TextComponentTranslation(this.getTranslationKey() + ".tooltip").getUnformattedText()
				);
	}
	...
	public final List<SkillBase> getSkills(Predicate<SkillBase> filter) {
		...
		SkillRegistry.getValues().stream().filter(s -> Config.isSkillInGroup(s, this)).filter(filter).forEach(s -> skills.add(s));
		return skills;
	}
	...
	public static final List<SkillGroup> getAll() {
		...
		Collections.sort(groups, (a, b) -> a == b ? 0 : (a.priority == b.priority ? a.label.compareTo(b.label) : b.priority - a.priority));
